...
/* ISSUE: i send data from the slave and when i recieve it it trigger the NACK recieved data somehting is sending a nack*/
...
ISR(TWI_vect)
{
	...
	switch(tw_status)
	{
		...
		{
			//init_variables();
			//twi_master_sla_send_address(AUTOPILOT_ADDRESS, TWI_READ);		
			break;
		}
...
//messagePacketHeader.data[0] == SET_HEADING) && (messagePacketHeader.data[1] == 0x01) && (messagePacketHeader.data[2] == 0x01